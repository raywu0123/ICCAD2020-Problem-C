...
from graph_preprocessing.constants import SINGLE_BIT_INDEX, BIT_INDEX_TYPE, WireKey
from graph_preprocessing.utils import extract_bitwidth, is_single_bit, make_wire_key
...
class Circuit:
    ...
    def __init__(self, gv_info, std_cell_info):
        ...
        self.cells, self.cell_id_to_cell = self.register_cells(gv_info, std_cell_info)
        ...
        for io in gv_info.io:
            if 'bitwidth' in io:
                ...
                for index in range(min(io.bitwidth), max(io.bitwidth) + 1):
                    wire_key = make_wire_key(io.id, index)
                    self.register_wire(wire_key)
            else:
                for idd in io.ids:
                    wire_key = make_wire_key(idd, SINGLE_BIT_INDEX)
                    self.register_wire(wire_key)
    ...
    def register_assigns(self, gv_info):
        ...
        for assign in gv_info.assign:
            ...
            if lhs_name in self.buses:
                ...
                lhs_key = make_wire_key(lhs_name, lhs_bitwidth[0])
            else:
                lhs_key = make_wire_key(lhs_name, SINGLE_BIT_INDEX)
            if rhs_name in self.buses:
                assert is_single_bit(rhs_bitwidth)
                rhs_key = make_wire_key(rhs_name, rhs_bitwidth[0])
            else:
                rhs_key = make_wire_key(rhs_name, SINGLE_BIT_INDEX)
    ...
    def register_cells(self, gv_info, std_cell_info) -> Tuple[dict, dict]:
        cells = {}
        for cell in gv_info.cells:
            cell_spec = self.get_cell_spec(cell.cell_type, std_cell_info)
    ...
    def from_parameter_string(self, p: str):
        ...
        if wire_name in self.buses:
            ...
            wire_key = make_wire_key(wire_name, bitwidth[0])
        else:
            wire_key = make_wire_key(wire_name, SINGLE_BIT_INDEX)
        return pin_name, wire_key

    @staticmethod
    def get_cell_spec(cell_type: str, std_cell_info):
        if cell_type in std_cell_info.primitives:
            return std_cell_info.primitives[cell_type]
        elif cell_type in std_cell_info.modules:
            return std_cell_info.modules[cell_type]
        else:
            return None
    ...
    def finalize(self, input_wires: Set[WireKey]):
        input_candidate_wires = {
            wirekey
            for wirekey, inputs in self.wire_inputs.items() if len(inputs) == 0
        }
        floating_inputs = input_candidate_wires.difference(input_wires).difference({("1'b0", 0), ("1'b1", 0)})
        if not len(floating_inputs) == 0:
            raise ValueError(f"{len(floating_inputs)} floating inputs: {floating_inputs}")

        self.input_wires.update(input_wires)
