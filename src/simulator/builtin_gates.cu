...
__host__ __device__ char nand_logic(
    Transition** data, unsigned int num_inputs, const unsigned int index, const char* table, unsigned int table_row_num
) {
    bool is_all_one = true, has_zero = false;
    for (int i = 1; i < num_inputs + 1; i++) {
        const auto& value = data[i][index].value;
...
__host__ __device__ char not_logic(
    ...
    const auto& value = data[1][index].value;
...
__host__ __device__ void and_gate_fn(
    ...
    stepping_algorithm(data, capacity, table, table_row_num, num_inputs, and_logic);
}
__host__ __device__ void or_gate_fn(
    ...
    stepping_algorithm(data, capacity, table, table_row_num, num_inputs, or_logic);
}
__host__ __device__ void xor_gate_fn(
    ...
    stepping_algorithm(data, capacity, table, table_row_num, num_inputs, xor_logic);
}
__host__ __device__ void nand_gate_fn(
    ...
    stepping_algorithm(data, capacity, table, table_row_num, num_inputs, nand_logic);
}
__host__ __device__ void nor_gate_fn(
    ...
    stepping_algorithm(data, capacity, table, table_row_num, num_inputs, nor_logic);
}
__host__ __device__ void xnor_gate_fn(
    ...
    stepping_algorithm(data, capacity, table, table_row_num, num_inputs, xnor_logic);
}
__host__ __device__ void not_gate_fn(
    ...
    stepping_algorithm(data, capacity, table, table_row_num, num_inputs, not_logic);
}
__host__ __device__ void buf_gate_fn(
    ...
    stepping_algorithm(data, capacity, table, table_row_num, num_inputs, buf_logic);
}
...
__host__ __device__ void primitive_gate_fn(
    ...
    stepping_algorithm(data, capacity, table, table_row_num, num_inputs, primitive_logic);
};
